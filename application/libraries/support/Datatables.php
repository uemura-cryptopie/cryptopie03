	...
	class Datatables
	{
			...
			if ($this->ci->input->post('order'))
				foreach ($this->ci->input->post('order') as $key)
					if($this->check_cType()) {
						...
						$str = $Data[$key['column']]['data'];
						$this->ci->db->order_by($this->table.'.'.substr($str, 0, strcspn($str, ',')), $key['dir']);
					} else {
						$str = $this->columns[$key['column']];
						$this->ci->db->order_by($this->table.'.'.substr($str, 0, strcspn($str, ',')) , $key['dir']);
					}
		...
		*/
		private function get_filtering()
		{
			...
			if($sSearch != '')
				// カンマを含んだ数値を数値型に変換
				if (is_numeric(str_replace(',', '', $sSearch))) {
		...
		private function produce_output($output, $charset)
		{
			...
			if($output == 'json')
			{
				...
				(
					...
					'recordsTotal'        => $iTotal,
					'recordsFiltered'     => ($iFilteredTotal === false) ? 0 : $iFilteredTotal,
					'data'                => $aaData
				);
		...
		*/
		private function get_total_results($filtering = FALSE)
		{
			...
			// echo "<pre>";var_dump($query);echo __LINE__.'行目';echo "</pre><br>";
			// echo "<pre>";var_dump($this->ci->db->last_query());echo __LINE__.'行目';echo "</pre><br>";
			return ($query) ? $query->num_rows() : $query;
		}
