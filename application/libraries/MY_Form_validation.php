...
class MY_Form_validation extends CI_Form_validation {
    ...
    function convert($str, $val)
    {
        ...
        switch($val)
        {
            ...
            case 'single_katakana': // 半角カタカナ
                return mb_convert_kana($str, 'khs');
                break;
            case 'phone': // 電話番号
    ...
    */
    ...
    function all_katakana($str)
    {
        ...
        $str = mb_convert_encoding($str, 'UTF-8');
        return ( ! preg_match("/^[ァ-ヾｱ-ﾝﾞﾟ\s　]+$/u", $str)) ? FALSE : TRUE;
    }
    ...
    function check_password($str) {
        ...
        if (!empty($password)) {
            ...
            $password = $this->CI->common->encrypt($password);
            $isValidPassword = $this->user_table->checkDuplicate(array('id'=>$this->CI->login_info['user_id'],'password' => $password), false);
            
            if (!$isValidPassword) {
                $this->_errorMessage = 'パスワードが正しくありません';
                return false;
            }
