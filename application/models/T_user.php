...
class T_user extends MY_Model {
	...
	public function checkSecurity($loginId,$userId) {
		if(empty($loginId) ||empty($userId)) {
			return false;
		}
		...
		if(empty($userLogin[0]['id'])) {
			...
			$where = 'id = '.$userId;
			$dataUser = $this->get_record($where , true);
			if(empty($dataUser[0]['id'] )|| $dataUser[0]['mail'] != $userLogin[0]['email']) {
				return false;
			}
		}
		return true;
